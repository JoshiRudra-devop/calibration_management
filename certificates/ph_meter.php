...
$pageTitle  = 'pH Meter Calibration';
$activePage = 'certificate';
include __DIR__ . '/../includes/header.php';

$db = getDB();
$stmt = $db->prepare("SELECT * FROM instrument_types WHERE slug = 'ph_meter' LIMIT 1");
$stmt->execute();
...
<div class="container">
    <h2 class="centered">   PH METER CALIBRATION CERTIFICATE</h2>
    <form id="calibrationForm">
      ...
      <div class="title_input_pair">
        <label for="make">Make:</label>
          <input type="text" id="make" value="HANNA" required readonly>
      </div>
      <div class="title_input_pair">
  ...
  <script>
    ...
    window.INSTRUMENT_SLUG = 'ph_meter';
  </script>
  <script>
    ...
    window.addCertificateDetails = function(doc, details) {
      ...
      doc.autoTable({
        head: [['SR NO', 'STANDARD BUFFER SOLUTION PH', 'TEST METER READING PH', 'ERROR']],
        body: calibrationData,
        startY: tableStartY + 0,
        styles: {   
          fontSize: 8,
          textColor:[0,0,0],
          lineColor:[87, 86, 85],
