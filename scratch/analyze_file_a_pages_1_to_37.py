...
for p_block in pages_raw:
    ...
    for i, l in enumerate(lines):
        if any(k in l for k in ['Calibrated Item', 'Description of Item', 'Nomenclature', 'Name of Instrument']):
            for k in range(i, min(i+4, len(lines))):
                text_k = lines[k].strip()
                if text_k and not any(k2 in text_k for k2 in ['Calibrated Item', 'Description of Item', 'Nomenclature']):
                    eq_name = text_k
                    break
